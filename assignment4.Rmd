...
#I found out that mean_attempt has the highest correlation with mean_correct followed by prior_percent_correct.
...
```{r}
T1<-read.csv("TCprograms.csv", sep = "", header = TRUE)
T2 <- T1[-1]
T2 <- scale(T2, center = TRUE)
tc.pca <- prcomp(T2, scale = TRUE)
plot(tc.pca, type = "lines")
COR4 <- cor(T2)
corrplot(COR4, method = "color", tl.pos="lt", type="upper", tl.col="black", tl.cex = 0.5)
summary(tc.pca)
```
