...
#make the name and read it 

D1 <- read.csv("Assistments-confidence.csv") %>% select(-id) 
...
# selcet and make them mean correct
...
```{r}
#read the file and name it D5
D5<- read.table("TCprograms.csv", header = TRUE, sep = "")



#make them clear and eliminate
...
corrplot(COR3, order="AOE", method="square", tl.pos="lt", type="upper",        
tl.col="green", tl.cex=0.1, tl.srt=20, 
        addCoef.col="black", addCoefasPercent = TRUE,
        sig.level=0.50, insig = "blank")
scale(D6,center = TRUE)

#To convert this into variance accounted for we can square it, these numbers are proportional to the eigenvalue

pca3<- prcomp(D6, scale = TRUE)

#get data frame 

D7<- as.data.frame(pca3$rotation)

#summary them 

summary(pca3)

# get the summary and plot 
