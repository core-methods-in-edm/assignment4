...
Based on the graph and , I deccide to remove pc6, pc5, pc4 because, for the stand deviation, they are less than 1. 
## Part II
...
```{r}
DF1 <- read.csv("TCprograms.csv", sep = "", header = TRUE)
...
corrplot(COR3, order="AOE", method="circle", tl.pos="lt", type="full",        
...
tl.col="black", tl.cex=0.3, tl.srt=45, 
        sig.level=0.50, insig = "blank")
```
# based on the information, we know some of them may belong to same depertments and some may have common field or knowledge. 
...
```
# there is no varience which is less than one. 
```{r}
pca2$rotation
