...
I will drop and remove 3,4,5,6.
...
biplot(pca, cex = 0.5, arrow.len = 0.01)
...
PC1 is more about policy, politics, leadership and PC2 is more about health, neuroscience,physiology. I feel that the variances number make similar programs closely relate to each other.

```{r}

TC1 <- read.csv("TCprograms.csv", header = TRUE, sep = " ")
TC1 <- subset(TC1, select = -program)
COR3 <- cor(TC1)
corrplot(COR3, order="AOE", method="circle", tl.pos="lt", type="upper",        
tl.col="black", tl.cex=0.6, tl.srt=45, 
        addCoef.col="black", addCoefasPercent = TRUE,
        sig.level=0.50, insig = "blank")
pca1 <- prcomp(TC1,scale. = TRUE, center = TRUE)
summary(pca1)
plot(pca1,type = "lines")
pca1$rotation
loadings1 <- abs(pca1$rotation)
sweep(loadings, 2, colSums(loadings1), "/")
biplot(pca1,cex = 0.2, arrow.len = 0.01)
```
