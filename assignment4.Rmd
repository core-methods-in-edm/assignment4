...
plot(D4)
COR2 <- cor(D4)
corrplot(COR2, order = "AOE", method = "circle", tl.pos = "lt", type = "upper", tl.col = "black", tl.cex = 0.6, tl.srt = 45,
         addCoef.col = "black", addCoefasPercent = TRUE,
         sig.level = 0.50, insig = "blank")
...
```{r}
d1 <- read.csv('humor_data.csv')[,1:32]
cor <- cor(d1)
corrplot(cor, order="AOE", method="circle", tl.pos="lt", type="upper",        
tl.col="black", tl.cex=0.6, tl.srt=45, 
        addCoef.col="black", addCoefasPercent = TRUE,
...
d4 <- cbind(d3, read.csv('humor_data.csv')[,33:36])
cor2 <- cor(d4)
