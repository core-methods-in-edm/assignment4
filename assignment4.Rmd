...
```{r}
D1 <- read.table("C:/Users/Nino Zhang/Documents/Nino Zhang/study at TC/fall 2016/EDM/assig6/assignment4/Assistments-confidence.csv", header=TRUE, 
  	sep=",", row.names="id")
...
From the variance vs plot, we can see that component 6 has the least variance, we might want to drop that one. 
```{r}
...
# but after doing the pca vs dropped data correlation plot, we can see that pc6 has a pretty high correlation with others, if we drop that, there might be an information loss. 
# Also, correct_mean has some high correlations w/ others as well, prolly not a good idea to drop it
# Maybe we can drop pca 4 and 5, there aren't many high corr in there
```
