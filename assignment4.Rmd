...
View(data)
cor.data <- cor(data)
corrplot(cor.data, order="AOE", method="circle", tl.pos="lt", type="upper", tl.col="black", tl.cex=0.6, tl.srt=45, 
        addCoef.col="black", addCoefasPercent = TRUE,
        sig.level=0.50, insig = "blank")

d.scaled <- scale(data[1:32], center = TRUE)
pca.data <- prcomp(d.scaled, scale = TRUE)

biplot(pca.data)
plot(pca.data, type = "lines")
```
The PCA seems to have been effective, becuase after 5 principle components the explained variance is almost zero, which is consistent with the theory behind this particular dataset (which includd 4 subscale scores to explain the raw humor survey data). 