...
corrplot(COR2, order = "AOE", method = "color", tl.pos = "lt", type = "upper", tl.col = "black", tl.cex =0.5, tl.srt = 90, addCoef.col = "orange", addCoefasPercent = TRUE, sig.level = 0.50, insig = "blank")
...
# Part III  
```{r}
TCProgram<- read.csv("TCprograms.csv", sep = "")
row.names(TCProgram) <- TCProgram$program
TCProgram$program <-NULL

TCProgram <-scale(TCProgram, center = TRUE)

pcaTCProgram <-prcomp(TCProgram, scale = TRUE)

summary(pcaTCProgram)
plot(pcaTCProgram, type = "lines" )
