...
library(tidyr)
library(dplyr)
D0<-read.csv("~/Desktop/hudk4050/assignment4/Assistments-confidence.csv")
D1 <- select(D0,-id)
...
```{r}
K1 <- read.csv("~/Desktop/hudk4050/assignment4/TCprograms.csv", sep="")
library(dplyr)
library(corrplot)
...
# I will drop everything after pc10, because they only accounts for a small proportion of variance。
#new data frame after pca
K3<-data.frame(pca1$x)
K4<-select(K3,c(1:10))
#COR4<-cor(K4)
#corrplot4<-corrplot(COR4, order="AOE", method="circle", tl.pos="lt", type="upper",        
#tl.col="black", tl.cex=0.6, tl.srt=45, 
        #addCoef.col="black", addCoefasPercent = TRUE,
        #sig.level=0.50, insig = "blank")
# zero componet between each pc, how to convert pc back to variable？
pca1$rotation
