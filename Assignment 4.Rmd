...
Cluster 1 has a higher motivation at first but after week 3, its motivation decreases.
Cluster 2 has a lower motivation at first but after week 3, its motivation increases.
...
ggplot(K6, aes(week, avg, colour = cluster)) + geom_line() + xlab("Week") + ylab("Average Motivation")
```
I think the clutsering with 3 groups is more informative, because 3 groups we can see more detailed motivation changes in different groups.
...
```{r}
K1 <- read.csv("HUDK405020-cluster.csv")
...
K4 <- data.frame(K3,fit$cluster)
names(K4) <- c("1","2", "cluster")
K5 <- gather(K4, "location", "value", 1:2)
K6 <- K5 %>% group_by(location, cluster) %>% summarise(avg = mean(value))
K6$cluster <- as.factor(K6$cluster)
K6$location <- as.numeric(K6$location)
library(ggplot2)
ggplot(K6, aes(location, avg, colour = cluster)) + geom_line() + xlab("Value") + ylab("Average Location")
K61 <- K6
```
```{r}
K1 <- read.csv("HUDK405020-cluster.csv")
...
K4 <- data.frame(K3,fit$cluster)
names(K4) <- c("1","2","3","4","5","6","cluster")
K5 <- gather(K4, "study", "value", 1:6)
K6 <- K5 %>% group_by(study, cluster) %>% summarise(avg = mean(value))
K6$cluster <- as.factor(K6$cluster)
K6$study <- as.numeric(K6$study)
library(ggplot2)
ggplot(K6, aes(study, avg, colour = cluster)) + geom_line() + xlab("Value") + ylab("Average Location")
K62 <- K6
```
...
```{r}
ggplot(K61, aes(location, avg, colour = cluster)) + geom_line() + xlab("value") + ylab("comprehensive value") +
   geom_line(data = K62, aes(study, avg, colour = cluster))
```
