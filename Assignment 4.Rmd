...
fit3b$tot.withinss
fit3c$tot.withinss
ML <- data.frame(M1$compare.features,M1$math.accuracy,M1$planner.use,M1$enjoy.discuss,M1$enjoy.group,M1$meet.deadline, fit2c$cluster, M1$lat,M1$long, fit3a$cluster)   #Combine everything together
pairs(ML)
...
Below are my original work, I did try myself and did not just copy the solution :)
```{r}
DF1 <- read.csv("HUDK405020-cluster.csv")
DF2 <- DF1 %>% select(lat, long) %>% scale
DF2 <- data.frame(DF2)
fit1 <- kmeans(DF2,2)
fit1$cluster

DF3 <- data.frame(DF2, fit1$cluster)
names(DF3) <- c("lat", "long", "clusterlocation")
DF3$cluster <- as.factor(DF3$cluster)
ggplot(DF3, aes(lat, long, colour=clusterlocation)) + geom_point()
```
```{r}
#what is the meaning of "their answers to the questions"
DF4 <- DF1 %>% select("math.accuracy") %>% scale
DF4 <- data.frame(DF4)
fit2 <- kmeans(DF4,2)
fit2$cluster

DF5 <- data.frame(DF4, fit2$cluster)
names(DF5) <- c("math.accuracy", "clusteraccuracy")
DF5$cluster <- as.factor(DF5$cluster)
ggplot(DF5, aes(DF1$id,math.accuracy, colour=clusteraccuracy)) + geom_line()
```
```{r}
ggplot(DF5, aes(DF1$id, math.accuracy,colour=clusteraccuracy, shape=as.factor(DF3$clusterlocation))) + 
  geom_line() + xlab("ID") + ylab("Math.Accuracy")
```
