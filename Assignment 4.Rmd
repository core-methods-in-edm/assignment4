...
fit.1 <- kmeans(DT1,2)
DT2 <- data.frame(DT1, fit.1$cluster)
names(DT2)[3] <- "cluster"
DT3 <- gather(DT2,"location","specific",1:2)
DT4 <- DT3 %>% group_by(location, cluster) %>% summarise(avg = mean(specific))
```
...
DT5 <- select(DT, 4:9)
fit.2 <- kmeans(DT5,2)
DT6 <- data.frame(DT5, fit.2$cluster)
names(DT6)[7] <- "cluster"
DT7 <- gather(DT6, "questions", "answers", 1:6)
DT8 <- DT7 %>% group_by(questions, cluster) %>% summarise(avg = mean(answers))
DT8$questions <- as.numeric(as.factor(DT8$questions))
DT8$cluster <- as.factor(DT8$cluster)
...
ggplot(DT8, aes(questions, avg, colour = cluster)) + geom_line() + xlab("Week") + ylab("Average Motivation")
```
