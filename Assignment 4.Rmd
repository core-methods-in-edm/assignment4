...
A1 <- read.csv("HUDK405020-cluster.csv", header = TRUE)
A2 <- select(A1, 2:3) %>% scale()
A3 <- scale(A2)
A3 <- as.data.frame(A3)
...
A4 <- data.frame(A1, fit2$cluster, fit3$cluster)

names(A4)[10] = "cluster1"
names(A4)[11] = "cluster2"

ggplot(A4, aes(lat,long, col=cluster1))+ geom_point()
ggplot(A4, aes(enjoy.group,enjoy.discuss, col=cluster2))+geom_point()
