...
fita <- kmeans(K3,centers=2)
fitb <- kmeans(K3,centers=2)
fitc <- kmeans(K3,centers=2)
...
K4<-data.frame(K3,fita$cluster, fitb$cluster, fitc$cluster)
...
```{r}
# Cluster 1 has higher motivation than Cluster 2 for all 4(week 1-4) weeks.But the average motivation of Cluster 2 exceeds that of cluster 1 in the final week. The clusters tend to follow opposite trends in week-by-week motivation trends.For example,the average motivation of cluster 1 decreases from week 1 to week 2 while cluster 2 increases. But from the week 4 to week 5, cluster 1 continues to decline in line with last week's trend, while cluster 2 continues to rise in line with last week's trend
```
...
LL2 <- data.frame(LL1, fit3a$cluster)
names(LL2) <- c("lat","long","location_cluster")
...
# It seems that there are geographical patterns that correspond to the answers.
```
