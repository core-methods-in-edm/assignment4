...
K6$week <- as.numeric(substring(K6$week, 11, 11))
...
```{r}
df <- read.csv("HUDK405020-cluster.csv")
set.seed(100)
# use K-means to cluster the students first according location (lat/long)
df1 <- scale(na.omit(df[, 2:3]))
fit1 <- kmeans(df1, 2, nstart = 5)
# use K-means to cluster the students first according their answers to the questions
df2 <- scale(na.omit(df[, 4:9]))
fit2 <- kmeans(df2, 2, nstart = 5)
# each student should belong to two clusters
result <- data.frame(cbind(df, fit1$cluster, fit2$cluster))
result$fit1.cluster <- as.factor(result$fit1.cluster)
result$fit2.cluster <- as.factor(result$fit2.cluster)
```
...
```{r}
ggplot(result, aes(long, lat, color = fit1.cluster, shape = fit2.cluster)) +
  geom_point(size = 3) +
  xlab("Long") +
  ylab("Lat")
