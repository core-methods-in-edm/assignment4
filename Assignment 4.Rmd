...
```{r}
## I could see two different colored polylines.The cluster 1 was higher than cluster 2, it could represent group 1 gain the high average motivation than the group 2. The image appears to be two opposite creases with some regularity.
```
...
```{r}
## I think clustering the students into two groups would be more effective because we could see more information and we also could see differences between two groups more clearly.
```
...
names(D5)[10]="cluster1"
names(D5)[9]="cluster2"
D5$cluster1 <- as.factor(D5$cluster1)
D5$cluster2 <- as.factor(D5$cluster2)
D4 <- data.frame(D4, fit2$cluster)
...
## I don't think there are relevant geographic patterns, because there are no relevant parts between these two clusters.
```
