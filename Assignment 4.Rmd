...
** grouping by 2 is more informative, because there is less overlapping between clusters.
...
D2 <- scale(D1[,2:3])
fit3 <- kmeans(D2,centers=3)
D3<-data.frame(D2,fit3$cluster)
fviz_cluster(fit3, data = D2,
...
D4 <- scale(D1[,4:9])
fit4 <- kmeans(D4,centers=3)
D5<-data.frame(D4,fit4$cluster)
fviz_cluster(fit4, data = D4,
             palette = c("#2E9FDF", "#00AFBB", "#E7B800"), 
             geom = "point",
             ellipse.type = "convex", 
             ggtheme = theme_bw()
