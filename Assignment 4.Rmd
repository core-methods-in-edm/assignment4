...
The trends of two clusters have an inverse relationship and have one intersection. For cluster 1, it has an increasing trend which starts with a negative value and end with a positive value. For cluster 2, it decreases from a positive value to a negative one.
...
D4 <- scale(D3)
fit2 <- kmeans(D4,2)
D5 <- data.frame(D4, cluster_location=as.vector(fit2$cluster))
D5$cluster_location <- as.factor(D5$cluster_location)
ggplot(D5, aes(lat, long, colour = cluster_location)) + geom_line() + xlab("lat") + ylab("long")
```


```{r}
DF1 <- D1[,c(4,5,6,7,8,9)]
DF2 <- na.omit(DF1)
DF3 <- scale(DF2)
fit3 <- kmeans(DF3,2)
DF4 <- data.frame(DF3, cluster_answer=as.vector(fit3$cluster))
names(DF4) <- c("Q1","Q2","Q3","Q4","Q5","Q6","cluster_answer")
DF5 <- gather(DF4,"Question","Score",1:6)
DF6 <- DF5 %>% group_by(Question, cluster_answer) %>% summarise(avg_score = mean(Score))
DF6$Question <- as.numeric(DF6$Question)
DF6$cluster_answer <- as.factor(DF6$cluster_answer)
ggplot(DF6, aes(Question, avg_score, colour = cluster_answer, group = cluster_answer)) + geom_line() + xlab("Question") + ylab("Average Score")
```
...
```{r}
DF6<-data.frame(D1,D5$cluster_location,DF4$cluster_answer)
ggplot(DF4, aes(D1$id, D5$cluster_location,DF4$cluster_answer, colour = DF4$cluster_answer, group = DF4$cluster_answer))+ geom_line() 
```
