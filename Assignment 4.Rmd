...
#recoding those missing value to zero is based on subjective assumption that those people have no motivation. 
#removing those missing value leads to great loss of participants. 
...
K4 <- mutate(K3, cluster = fit$cluster)
...
```{r}
library(readr)
K6 <- mutate(K6,week=parse_number(week))
...
#cluster 1 is above cluster 2. Value higher than 0 is defined as cluster 1 except week5.
#Value lower than 0 is defined as cluster 2 except week 5
#? why week5 seems not fit in this cluster?
...
```{r}
E <- read.csv("HUDK405020-cluster.csv", header=T)
#cluster for lat/long)
E1 <- data.frame(scale(E[,c(2,3)]))
fit3 <- kmeans(E1,2)
E2 <- mutate(E1,cluster=fit3$cluster)
ggplot(E2, aes(lat, long, colour=cluster)) + geom_point()

#cluster according to their answers to the questions 
Q <- data.frame(scale(E[,4:9]))
fit4 <- kmeans(Q,2)
Q1 <- mutate(Q,cluster=fit4$cluster)
plot(Q1,col=Q1$cluster)
...
```{r}
EQ <- data.frame(E2,Q1$cluster)
ggplot(EQ, aes(lat,long, col=Q1.cluster))+ geom_point()
```
