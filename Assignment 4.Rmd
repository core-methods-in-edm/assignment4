...
If we recode them, we don't know what score we should replace to. If we remove them, the sample may be biased. 
...
For cluster 1, it is like a "M". For cluster 2, it is like a "W".
...
I think 3 clusters format is more informative. 
...
#according location cluster
```{r}
D2 <- select(D1, 2:3)
fit1 <- kmeans(D2, 2)
D3 <- data.frame(D2, fit1$cluster)
names(D3) <- c("lat","long","location_cluster")
ggplot(D3, aes(lat,long,col = location_cluster)) + geom_point()
```
#according answers to the questions
```{r}
D4 <- select(D1, 4:9)
...
```{r}
DL <- data.frame(D1$compare.features, D1$math.accuracy, D1$planner.use, D1$enjoy.discuss, D1$enjoy.group, D1$meet.deadline, fit1$cluster, D1$lat, D1$long, fit2g$cluster)
...
```{r}
table(DL$fit1.cluster, DL$fit2g.cluster)

#install.packages("vcd")
library(vcd)
P1 <- structable(fit1$cluster ~ fit2g$cluster)
