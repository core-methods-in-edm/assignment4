...
K6$fit.cluster <- as.factor(K6$fit.cluster)
...
ggplot(K6, aes(week, avg, colour = fit.cluster)) + geom_line() + xlab("Week") + ylab("Average Motivation")
...
2 cluster grouping would be more informative because the differences will be clearer.
...
```{r}
DA1 <- read.csv("HUDK405020-cluster.csv", header = TRUE)
DA2 <- select(DA1, 2:9) %>% scale()
DA3 <- select(DA1, 1:3) %>% scale()
DA4 <- select(DA1, 3:8) %>% scale()
fit1 <- kmeans(DA3, 2)
fit2 <- kmeans(DA4, 2)
fit3 <- kmeans(DA3, 2)
names(D4)[10]="cluster1"
names(D4)[11]="cluster2"
DA5 <- data.frame(DA2, fit1$cluster, fit2$cluster)
DA5$clusters <- paste(DA5$fit1.cluster, DA5$fit2.cluster)
DA5 <- DA5[, -c(9,10)]
```
...
#I don't think there are patterns that line up with the answers.
```
