...
If we remove missing values, then it is possible to get  biased conclusions or lose information.
If we recode
...
K6 <- K5 %>%group_by(week, cluster) %>% summarise(mean_motivation = mean(motivation))
...
ggplot(K6, aes(week, mean_motivation, colour = cluster)) + geom_line() + xlab("Week") + ylab("Average Motivation")
...
For cluster 1, as time goes by, the average motivation goes down as well. For cluster 2, the average motivation goest up. 
...
```{r}
#I don't think 3 clusters provide significantly more informative than 2 clusters. 
```
...
```{r}
setwd("/Users/jy/Desktop/HUDK 4050/Assignment_4/assignment4")
#cluster based on location
H1 <- read.csv(file = "HUDK405020-cluster.csv", header = TRUE)
H2 <- select(H1,2:3)
plot(H1$long, H1$lat)
H3 <- na.omit(H2)
H3 <- scale(H3)
...
#cluster based on other answers to the questions.
K1 <- read.csv(file = "HUDK405020-cluster.csv", header = TRUE)
K2 <- select(K1, 4:9)
K3 <- na.omit(K2)
K3 <- as.data.frame(scale(K3))
fit3 <- kmeans(K3, 5)

#Professor Lang's version of the assignment.
...
mosaic(P1, shade = TRUE, legend = TRUE, coloR = FALSE)

#Professor Lang's code
...
P2 <- structable(fit2c$cluster ~ fit3$cluster)
mosaic(P2, shade = T, legend = T)
```
