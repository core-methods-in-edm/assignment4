...
PCA chooses an axis that maximises variance of the given data. For example if the data points lie tighly in an oval around an axis that is 45 degrees to x and y, then the axis it will chooseis the 45 degree axis instead of x or y. Since, if it chooses x or y the variance decreases as the points will be projected on these axis. (DIRECTION that maximises the variance)
Second it chooses an axis that is orthogonal (90 degrees) to the chosen axis so as to get a 2D graph.
...
Corrplot package is used to plot some pretty correlation matrices (sometimes called correlograms)
...
Yes, if we had dropped the variable 6 we may have lost important information about mean correct since, pc5 has a slightly strong positive corelation with mean correct
...
The variables Q1 through Q32 were statements rated on a five point scale where 
1=Never or very rarely true, 
2=Rarely true, 
3= Sometimes true, 
4= Often true, 
5=Very often or always true (-1=did not select an answer). 
The exact statements were:
